...
export default function Home() {
  ...
  const handleBalanceChecked = (bal) => {
    setBalance(bal);
    setShowTopUp(bal === '0');
  };
  const handleTopUp = () => {
  ...
  const handleChainSelect = (selected) => setChain(selected);
  const handleAssetAmountChange = (type, value) => {
  ...
  return (
    <div>
      <Head>
        <title>DApp Cross-Chain Pagos</title>
      </Head>
      <main>
